...
from utils.config import MAX_LENGTH, TEMPERATURE, TOP_P, STOP_SEQUENCES, PLANNING_THRESHOLD, REFLECTION_ENABLED
...
class HierarchicalPlanner:
    ...
    async def plan_task(self, query: str, multimodal_context: Optional[dict] = None) -> List[Dict]:
        ...
        prompt = (
            ...
            f"Only generate a plan if the query length exceeds {PLANNING_THRESHOLD} characters or is complex."
        )
        ...
        try:
            ...
            logger.info(f"Generated plan in {duration:.2f} seconds: {plan}")
            return plan
    ...
    async def optimize_plan(self, plan: List[Dict], multimodal_context: Optional[dict] = None) -> List[Dict]:
        ...
        try:
            ...
            logger.info(f"Optimized plan in {duration:.2f} seconds: {optimized_plan}")
            return optimized_plan
        except Exception as e:
            logger.error(f"Failed to parse optimized plan: {e}")
            return plan
